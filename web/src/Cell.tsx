...
const STATUS_COLOURS: Record<string, string> = {
  Approved: 'green', Paid: 'green', Low: 'green',
  Pending: 'amber', 'In review': 'blue', 'Awaiting approval': 'amber', Medium: 'amber',
  Escalated: 'red', Rejected: 'red', High: 'red', None: '',
}
...
export function Cell({ f, r, isTitle }: { f: FieldSpec; r: Rec; isTitle?: boolean }) {
  ...
  if (v == null || v === '') return <span style={{ color: '#a19f9d' }}>—</span>
  if (isTitle) return <span className="link">{String(v)}</span>
  switch (f.type) {
    case 'boolean':
      if (['dev', 'uat', 'prod'].includes(f.name)) return <span className={`tgl ${v ? 'on' : ''}`} />
      return v ? <span className="pill red">Yes</span> : <span style={{ color: '#a19f9d' }}>No</span>
